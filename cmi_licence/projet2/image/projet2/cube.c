...
void initialisation(char *nom)
{
  if(nom==NULL)
    {
      printf("Erreur: Il n'y a pas de nom de fichier\n");
      exit(1);
    }

  strcpy(nom_fichier,nom/*"cube2.txt"*/);
...
void menu(char *nom)
{
  ...
  while(!fin)
	...
	case 3:
	  ...
	  /*xo=0.0; yo=0.0; zo=3.0;
	    phi=0.0; theta=M_PI/5; psi=M_PI/3;
	    
	    d=10.0;r=1.0;*/

	  initialiser_affichage();
	  //initialisation(nom_fichier);
	  afficher();
	  break;
