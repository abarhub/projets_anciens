...
void affiche_LS(void)
{
  ...
  while(l!=NULL)
	...
	if(l->crochet_ouvrant>0)
	  {
	    int i;
		for(i=0;i<l->crochet_ouvrant;i++)
			printf("[");
	  }
	...
	if(l->crochet_fermant>0)
	  {
	    int i;
		for(i=0;i<l->crochet_fermant;i++)
			printf("]");
	  }
...
void efface_ls(void)
{
  ...
  printf("Effacement de la table des symbols:\n");
  if(!table_symbol_vide())//table_symbol!=NULL)
    efface_table_symbol();
  
  gc();
  printf("Fin d'effacement\n");
}
...
EXPR *creer_variable(char *nom/*,int no*/,EXPR *indice,LS*ls)
{
  EXPR *e;
  ...
  complete_LS(ls,e,efface_expr,affiche_expr);
  e->ls=ls;
  e->marque=0;
