...
<BODY BGCOLOR="#FFFFFF"><PRE><Font face="Arial, Helvetica, sans-serif">#include <signal.h>
#include <stdio.h>
#include <stdlib.h>
#include <X11/Xlib.h>
#include <X11/Xutil.h>
        ...
        le cas pour les pseudocolors */
              ...
              if (!gagne)
                { if ( i > 100 && nbg > i+2) nbg=i+2;
                       /* bidouille pour aller plus vite lorsque c'est
satisfaisant */
          ...
          if (!gagne)
            { fprintf(stderr,"Il n'y a pas assez de nuances de gris pour
executer cette application\n");
              fprintf(stderr,"Mille excuses !\n");
              exit(1);
            }
        }
...
static int pere()
{
 ...
 signal(SIGCHLD,application_terminee);
  ...
  XSelectInput(canal,fen,
                ButtonPressMask   /* pour controler les zooms, et ? presser
bouton */
               |KeyPressMask      /* debug */
               |ExposureMask      /* rafraichissement */
               |ButtonReleaseMask /* pour controler les zooms, et ? presser
bouton */
